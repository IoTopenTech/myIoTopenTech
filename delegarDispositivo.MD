...
Existe también una Delegación pública, que crea un dispositivo delegado asignado al customer Public, de tal modo que se puede acceder directamente a su dashboard sin acreditarse en myIoT. Para diferenciarla de cualquier otra delegación, su nombre es D[nonmbreCustomer]__[idDispositivoDelegador]_publica (como los nombres de las delegaciones no pueden empezar con _ no existe la posibilidad de que el usuario pueda sobreescribirla). Esta delegación no consume créditos. Siempre tendrá el atributo reclamado como true para que se le pasen las telemetrías. No tendrá claiming data porque no puede ser reclamado (está asignado al customer public).
